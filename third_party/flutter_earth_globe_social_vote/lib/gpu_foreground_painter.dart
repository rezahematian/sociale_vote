...
class GpuForegroundPainter extends CustomPainter {
  ...
  void _drawPoint(Canvas canvas, PointRenderData point) {
    ...
    // Social Vote G5D:
    // screen-facing beacon anchored to the globe. This avoids the stretched
    // ellipse/blob effect near the limb while keeping geographic anchoring.
    const baseRadius = 150.0;
    final globeScale = radius / baseRadius;
    final depthScale = 0.78 + 0.34 * point.depth;

    final logicalRadius =
        (style.size * depthScale * globeScale).clamp(2.0, 9.5).toDouble();
    final alpha =
        (style.color.a * point.transitionProgress).clamp(0.0, 1.0).toDouble();

    final altitudeOffset = style.altitude * point.depth * 2.0 * globeScale;
    final drawPos = Offset(
      point.position2D.dx,
      point.position2D.dy - altitudeOffset,
    );

    final baseColor = style.color.withValues(alpha: alpha);

    final glowPaint = Paint()
      ..color = baseColor.withValues(alpha: alpha * 0.20)
      ..style = PaintingStyle.fill
      ..maskFilter = MaskFilter.blur(
        BlurStyle.normal,
        math.max(2.2, logicalRadius * 1.15),
      );
    canvas.drawCircle(drawPos, logicalRadius * 2.15, glowPaint);

    final separatorPaint = Paint()
      ..color = const Color(0xCC06101D)
      ..style = PaintingStyle.fill;
    canvas.drawCircle(drawPos, logicalRadius * 1.28, separatorPaint);

    final ringPaint = Paint()
      ..color = baseColor
      ..style = PaintingStyle.stroke
      ..strokeWidth = math.max(1.25, logicalRadius * 0.34)
      ..strokeCap = StrokeCap.round;
    canvas.drawCircle(drawPos, logicalRadius, ringPaint);
    ...
    canvas.drawCircle(
      drawPos,
      math.max(1.25, logicalRadius * 0.34),
      corePaint,
    );

    final highlightPaint = Paint()
      ..color = const Color(0xFFFFFFFF).withValues(alpha: alpha * 0.80)
      ..style = PaintingStyle.fill;
    canvas.drawCircle(
      drawPos.translate(-logicalRadius * 0.16, -logicalRadius * 0.18),
      math.max(0.55, logicalRadius * 0.10),
      highlightPaint,
    );
  }
