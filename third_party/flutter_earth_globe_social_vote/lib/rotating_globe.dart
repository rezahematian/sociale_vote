...
class RotatingGlobeState extends State<RotatingGlobe>
  ...
  // Raw-pointer tap arbitration for Social Vote country selection.
  // A Listener is used instead of a nested GestureDetector so InteractiveViewer
  // can keep ownership of rotate/pinch gestures without swallowing a real tap.
  static const double _tapMovementTolerance = 10.0;
  final Set<int> _tapPointers = <int>{};
  ...
  void initState() {
    ...
    // WEB-G1B: the package GPU sphere currently paints foreground/glow on
    // standard Flutter Web but can lose the Earth texture. Until the package
    // shader path is made fully web-safe, use its deterministic CPU sphere on
    // Web only. Android/iOS keep the faster GPU path unchanged.
    if (kIsWeb) {
  ...
  /// Convert a position local to this RotatingGlobe into geographic coordinates.
  ///
  /// Social Vote uses this from the outer Explore pointer arbiter so country
  /// selection does not depend on Flutter's nested gesture arena.
  GlobeCoordinates? coordinatesAtLocalPosition(Offset localPosition) {
    // The renderer can intentionally be larger than the visible square viewport.
    // The sphere itself is shifted inside that larger canvas by widget.alignment.
    // Country-tap hit testing must therefore use the *rendered* sphere center,
    // not the unshifted LayoutBuilder canvas center.
    final renderedSphereCenter = Offset(
      center.dx + (widget.alignment.x * center.dx),
      center.dy + (widget.alignment.y * center.dy),
    );

    return convert2DPointToSphereCoordinates(
      localPosition,
      renderedSphereCenter,
      convertedRadius(),
      rotationY,
      rotationZ,
    );
  ...
  /// Geographic coordinate currently displayed at the visual center
  /// of the sphere. Used by Social Vote for the seamless 3D -> 2D handoff.
  GlobeCoordinates? centerCoordinates() {
    final renderedSphereCenter = Offset(
      center.dx + (widget.alignment.x * center.dx),
      center.dy + (widget.alignment.y * center.dy),
    );

    return coordinatesAtLocalPosition(renderedSphereCenter);
  }

  /// Returns the visible point under [localPosition], if any.
  ///
  /// Social Vote uses this before Country hit-testing so a Poll/Post/News
  /// marker always owns the tap when it overlaps the Earth surface.
  String? pointIdAtLocalPosition(Offset localPosition) {
    if (_pointRenderData.isEmpty) {
      return null;
    }
  ...
  void onTapEvent(Offset localPosition) {
    clickPoint = localPosition;
    _clickNotifier.value = localPosition;
    // Don't call setState - the ValueNotifier will trigger foreground repaint.
    widget.onTap?.call(coordinatesAtLocalPosition(localPosition));
  }

  void _handleTapPointerDown(PointerDownEvent event) {
    _tapPointers.add(event.pointer);

    if (_tapPointers.length == 1) {
    ...
    // Two or more pointers means pinch/multi-touch, never a country tap.
    _tapCandidatePointer = null;
    _tapDownPosition = null;
    _tapCandidateMoved = true;
  }

  void _handleTapPointerMove(PointerMoveEvent event) {
    if (event.pointer != _tapCandidatePointer || _tapDownPosition == null) {
      return;
    }
  ...
  void _handleTapPointerUp(PointerUpEvent event) {
    final isConfirmedTap = event.pointer == _tapCandidatePointer &&
        !_tapCandidateMoved &&
        _tapDownPosition != null &&
        _tapPointers.length == 1;

    _tapPointers.remove(event.pointer);
    ...
    if (isConfirmedTap) {
      onTapEvent(event.localPosition);
    }
  }

  void _handleTapPointerCancel(PointerCancelEvent event) {
    _tapPointers.remove(event.pointer);
    ...
    return Stack(
      children: [
        ...
        LayoutBuilder(builder: (context, constraints) {
          if (widget.controller.background != null) {
            return RepaintBoundary(
              child: _buildBackground(constraints),
            );
          }

          if (!widget.showSpaceBackground) {
            return const SizedBox.shrink();
          }

          return RepaintBoundary(
            child: CustomPaint(
              painter: _SocialVoteSpaceBackgroundPainter(
                rotationY: rotationY,
                rotationZ: rotationZ,
            ...
            child: InteractiveViewer(
              ...
              onInteractionUpdate: (ScaleUpdateDetails details) {
                ...
                if (widget.lockVerticalRotation) {
                  ...
                  rotationX = requestedHomeTilt
                      .clamp(-maxHomeTilt, maxHomeTilt)
                      .toDouble();
                  rotationY = -rotationX;
                } else {
                  final maxExploreTilt = radians(
                  ...
                  final requestedExploreTilt = _lastRotationX +
                      (offset.dy / convertedRadius()) * panFactor;
                  rotationX = requestedExploreTilt
                      .clamp(-maxExploreTilt, maxExploreTilt)
                      .toDouble();
                  rotationY = -rotationX;
                }
              ...
              onInteractionEnd: (ScaleEndDetails details) {
                ...
                if (velocityMagnitude > 30) {
                  ...
                  if (widget.lockVerticalRotation) {
                    // Preserve the already-approved Home behaviour exactly:
                    // no vertical inertia and the original horizontal inertia.
                    _targetRotationX = rotationX;
                    _targetRotationY = rotationY;
                    _targetRotationZ =
                        rotationZ + _angularVelocityZ * velocityFactor;
                    _decelerationController.duration =
                        const Duration(milliseconds: 1200);
                  } else {
                    final maxExploreTilt = radians(
                      widget.maxVerticalTiltDegrees.clamp(0.0, 89.0).toDouble(),
                    );
                    final exploreInertiaStrength =
                        widget.inertiaStrength.clamp(0.0, 1.0).toDouble();

                    final requestedTargetTilt = rotationX +
                        _angularVelocityX *
                            velocityFactor *
                            exploreInertiaStrength;
                    _targetRotationX = requestedTargetTilt
                        .clamp(-maxExploreTilt, maxExploreTilt)
                        .toDouble();
              ...
              child: Listener(
                ...
                onPointerDown: _handleTapPointerDown,
                onPointerMove: _handleTapPointerMove,
                onPointerUp: _handleTapPointerUp,
                onPointerCancel: _handleTapPointerCancel,
                child: Listener(
...
class _SocialVoteSpaceBackgroundPainter extends CustomPainter {
  static final List<_SocialVoteSpaceStar> _stars = _buildStars();

  final double rotationY;
  final double rotationZ;
  final double zoom;

  const _SocialVoteSpaceBackgroundPainter({
    required this.rotationY,
    required this.rotationZ,
    required this.zoom,
  });

  static List<_SocialVoteSpaceStar> _buildStars() {
    final random = math.Random(20260813);
    return List<_SocialVoteSpaceStar>.generate(150, (index) {
      final brightStar = index % 19 == 0;
      return _SocialVoteSpaceStar(
        x: random.nextDouble(),
        y: random.nextDouble(),
        radius: brightStar
            ? 1.15 + random.nextDouble() * 0.75
            : 0.28 + random.nextDouble() * 0.72,
        brightness: brightStar
            ? 0.72 + random.nextDouble() * 0.28
            : 0.26 + random.nextDouble() * 0.48,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // A restrained Milky-Way-like haze. It is intentionally subtle so the
    // Earth and content markers stay readable.
    canvas.save();
    ...
    final hazeRect = Rect.fromCenter(
      ...
      height: math.max(70.0, size.height * 0.20),
    );
    ...
    final horizontalShift = (rotationZ / (2 * math.pi)) * size.width * 0.28;
    final verticalShift = (rotationY / math.pi) * size.height * 0.16;
    final zoomParallax = 1.0 + zoom.clamp(-0.22, 1.15).toDouble() * 0.025;

    for (final star in _stars) {
      var x =
          (star.x * size.width * zoomParallax + horizontalShift) % size.width;
      var y =
          (star.y * size.height * zoomParallax + verticalShift) % size.height;

      if (x < 0) x += size.width;
      if (y < 0) y += size.height;

      final blue = (225 + (star.warmth * 30)).round().clamp(0, 255).toInt();
      final red =
          (218 + ((1.0 - star.warmth) * 37)).round().clamp(0, 255).toInt();

      final starPaint = Paint()
        ..color = Color.fromARGB(
          (star.brightness * 255).round().clamp(0, 255).toInt(),
          red,
          235,
          blue,
        );
      ...
      if (star.radius > 1.1) {
        final glowPaint = Paint()
          ..color = starPaint.color.withValues(alpha: 0.10)
  ...
  @override
  bool shouldRepaint(
    covariant _SocialVoteSpaceBackgroundPainter oldDelegate,
  ) {
    return (rotationY - oldDelegate.rotationY).abs() > 0.001 ||
        (rotationZ - oldDelegate.rotationZ).abs() > 0.001 ||
        (zoom - oldDelegate.zoom).abs() > 0.001;
  }
}
