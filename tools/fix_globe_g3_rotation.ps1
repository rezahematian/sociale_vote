...
# Safety: the public G3 fields must already be present.
$requiredFields = @(
...
foreach ($field in $requiredFields) {
    if (-not $text.Contains($field)) {
        throw "Patch G3 incompleta a monte: manca '$field'. Non modifico il file."
    }
}

$backup = "$file.before_g3_rotation_fix"
...
function Replace-Once {
    ...
    if ($count -ne 1) {
        throw "Patch '$Label' non sicura: atteso 1 match, trovati $count."
    }
...
# 1. Helper per il limite verticale.
Replace-Once `
...
# 2. Durata inerzia configurabile.
Replace-Once `
...
# 3. Clamp anche durante l'inerzia.
Replace-Once `
...
# 4. Anche focus programmatico rispetta il profilo.
Replace-Once `
...
# 5. Questo e il fix principale:
# il drag verticale originale ruotava X e Y senza limiti.
Replace-Once `
...
# 7. Niente inerzia verticale in Home.
Replace-Once `
...
# 8. Anche il target finale dell'inerzia viene limitato.
Replace-Once `
...
Write-Host ""
Write-Host "=== VERIFICA PATCH G3 ==="

$checks = @(
    '_maxInteractiveTiltRadians',
    'if (widget.lockVerticalRotation)',
    'widget.decelerationDuration',
    'widget.inertiaStrength'
)

foreach ($check in $checks) {
    $matches = Select-String -Path $file -Pattern ([regex]::Escape($check))
    Write-Host "$check -> $($matches.Count) match"
}

Write-Host ""
Write-Host "Patch G3 rotazione applicata."
